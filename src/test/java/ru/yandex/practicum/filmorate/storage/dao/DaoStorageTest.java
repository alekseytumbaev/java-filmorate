...
class DaoStorageTest {
    ...
    public void filmAndLikeStorage() {
        ...
        assertEquals("Фильм не обновлен", 2L, updatedFilm.getMpa().getId());
        ...
        Film film1 = new Film("Film name 2", new MotionPictureAssociation(3, "PG-13"),
                "Film descr 2", now, 80);
        film1.getGenres().add(new Genre(2, "Драма"));
        filmStorage.add(film1);

        assertEquals("Получено неправильное количество фильмов", 2, filmStorage.getAll().size());


        List<Film> mostPopularFilms = new ArrayList<>(filmStorage.getOrderedByLikesAcs(2));
        assertEquals("Фильмы неправильно отсортированы по популярности", film, mostPopularFilms.get(0));
        ...
        assertEquals("Фильмы неправильно отсортированы по популярности", film1, mostPopularFilms1.get(1));
    }
    ...
    public void userAndFriendshipStorage() {
        ...
        assertEquals("Пользователь не обновлен", "updated_login", updatedUser.getLogin());
        ...
        assertEquals("Пользователь не добавлен в друзья", List.of(friend),
                friendshipStorage.getFriendsByUserIdAndFriendshipStatus(userId, CONFIRMED));
        ...
        assertEquals("Общий друг не найден", List.of(friend),
                friendshipStorage.getMutualFriends(userId, friendId1));
